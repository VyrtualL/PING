...
function isValidProgram(program_) {
  let ret = ''
  let count = 0
  for (let i = 0; i < program_.length && ret === ''; i++) {
    switch (program_[i]) {
    case '[':
      count++
      break
    case ']':
      if (count == 0) {
        ret = '[ERROR] Missing left bracket'
        break
      }
      count--
      break
  ...
  if (count != 0 && ret === '') {
    ret = '[ERROR] Missing right bracket'
  }
  return ret
}

function interpret(program_, input_) {
  resetState()
  program = program_
  input = input_
  const err = isValidProgram(program)
  if (err != '') {
    return err
  }
  let end = false
